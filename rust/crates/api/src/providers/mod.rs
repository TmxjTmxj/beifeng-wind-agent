...
const MODEL_REGISTRY: &[(&str, ProviderMetadata)] = &[
    ...
    ("grok-2", XAI_METADATA),
    ("kimi", DASHSCOPE_METADATA),
    ("deepseek-chat", DEEPSEEK_METADATA),
    ("deepseek-reasoner", DEEPSEEK_METADATA),
    ("sdu-deepseek", DEEPSEEK_METADATA),
    ("wind-deepseek", DEEPSEEK_METADATA),
];
        ...
        .find_map(|(alias, metadata)| {
            (*alias == lower).then_some(match metadata.provider {
                ...
                ProviderKind::DeepSeek => match *alias {
                    "deepseek-chat" | "wind-deepseek" => "deepseek/deepseek-chat",
...
/// Data-driven prefix routing table for [`metadata_for_model`].
/// Each entry maps a set of model-name prefixes to the provider that handles
/// them, with an optional case-insensitive match. Order matters: first match
/// wins. The table is checked *after* the explicit `MODEL_REGISTRY` alias
/// lookup, so aliases like "grok" or "wind-deepseek" are resolved before we
/// ever reach these prefix rules.
struct ProviderPrefixRoute {
    prefixes: &'static [&'static str],
    metadata: ProviderMetadata,
    case_insensitive: bool,
}

const PROVIDER_PREFIX_ROUTES: &[ProviderPrefixRoute] = &[
    ProviderPrefixRoute {
    ...
    ProviderPrefixRoute {
        prefixes: &["grok"],
        metadata: XAI_METADATA,
        case_insensitive: false,
    },
    // DeepSeek models: deepseek/*, deepseek-*, SDU-AI/*, Ali-dashscope/deepseek-*.
    // Case-insensitive to catch "SDU-AI/DeepSeek-V4-Flash" etc.
    ProviderPrefixRoute {
        prefixes: &[
            "deepseek/",
            "deepseek-",
            "sdu-ai/",
            "ali-dashscope/deepseek-",
        ],
        metadata: DEEPSEEK_METADATA,
        case_insensitive: true,
    },
    // Explicit provider-namespaced OpenAI models (e.g. "openai/gpt-4.1-mini")
    // and bare gpt- prefix. Must route here regardless of auth env vars.
    ProviderPrefixRoute {
        prefixes: &["openai/", "gpt-"],
        metadata: OPENAI_METADATA,
        case_insensitive: false,
    },
    // Alibaba DashScope compatible-mode: qwen/* and bare qwen-* model names
    // (qwen-max, qwen-plus, qwen-turbo, qwen-qwq, etc.). Uses OpenAi
    // provider kind because DashScope speaks the OpenAI REST shape.
    ProviderPrefixRoute {
        prefixes: &["qwen/", "qwen-"],
        metadata: DASHSCOPE_METADATA,
        case_insensitive: false,
    },
...
/// Auth-sniffer key order for [`detect_provider_kind`].
/// When no model prefix matches, we check API keys in this priority order.
const AUTH_SNIFFER_KEY_ORDER: &[(&str, ProviderKind)] = &[
    ("OPENAI_API_KEY", ProviderKind::OpenAi),
    ("XAI_API_KEY", ProviderKind::Xai),
...
pub fn metadata_for_model(model: &str) -> Option<ProviderMetadata> {
    ...
    for route in PROVIDER_PREFIX_ROUTES {
        let matches = if route.case_insensitive {
            starts_with_any_lower(&canonical, route.prefixes)
        } else {
            starts_with_any(&canonical, route.prefixes)
        };
        if matches {
...
fn starts_with_any_lower(value: &str, prefixes: &[&str]) -> bool {
    let lower = value.to_ascii_lowercase();
    starts_with_any(&lower, prefixes)
}
...
pub fn provider_diagnostics_for_model(model: &str) -> ProviderDiagnostics {
    ...
    let metadata = metadata_for_model(&resolved_model)
        .unwrap_or_else(|| metadata_for_provider_kind(detect_provider_kind(&resolved_model)));
...
pub fn detect_provider_kind(model: &str) -> ProviderKind {
    ...
    // Table-driven auth-key sniffing for non-Anthropic providers.
    for (env_key, provider_kind) in AUTH_SNIFFER_KEY_ORDER {
        if openai_compat::has_api_key(env_key) {
...
pub fn provider_capabilities_for_model(model: &str) -> ProviderCapabilityReport {
    let resolved_model = resolve_model_alias(model);
    let metadata = metadata_for_model(&resolved_model).unwrap_or_else(|| {
        let provider = detect_provider_kind(&resolved_model);
    ...
    let (
        ...
        ProviderKind::OpenAi | ProviderKind::DeepSeek => (
            ...
            ProviderFeatureSupport::Supported,
            ProviderFeatureSupport::Supported,
            if openai_compat::model_requires_reasoning_content_in_history(&resolved_model) {
                ProviderFeatureSupport::Supported
            } else {
                ProviderFeatureSupport::Unsupported
            },
...
pub fn provider_diagnostics_for_request(request: &MessageRequest) -> Vec<ProviderDiagnostic> {
    ...
    let resolved_model = resolve_model_alias(&request.model);
    ...
    if openai_compat::is_reasoning_model(&resolved_model)
...
fn metadata_for_provider_kind(provider: ProviderKind) -> ProviderMetadata {
    match provider {
        ...
        ProviderKind::OpenAi => OPENAI_METADATA,
        ProviderKind::DeepSeek => DEEPSEEK_METADATA,
    }
}
...
pub fn model_token_limit(model: &str) -> Option<ModelTokenLimit> {
    ...
    match base_model {
        ...
        "DeepSeek-V4-Flash" | "deepseek-v4-flash" => Some(ModelTokenLimit {
            max_output_tokens: 32_768,
            context_window_tokens: 128_000,
        }),
        _ => None,
...
const FOREIGN_PROVIDER_ENV_VARS: &[(&str, &str, &str)] = &[
    ...
    (
        "DEEPSEEK_API_KEY",
        "DeepSeek",
        "use `--model deepseek-chat`, `--model deepseek-reasoner`, `--model wind-deepseek`, or a `deepseek/*` model name so the prefix router selects the DeepSeek backend",
    ),
];
...
mod tests {
    ...
    fn resolves_grok_aliases() {
        ...
        assert_eq!(
            resolve_model_alias("wind-deepseek"),
        ...
        assert_eq!(
            resolve_model_alias("sdu-deepseek"),
    ...
    fn detects_provider_from_model_name_first() {
        ...
        assert_eq!(
            detect_provider_kind("wind-deepseek"),
        ...
        assert_eq!(
            detect_provider_kind("SDU-AI/DeepSeek-V4-Flash"),
            ProviderKind::DeepSeek
        );
    }

    #[test]
    fn routes_core_provider_aliases_without_env_sniffing() {
        let cases = [
            ("deepseek-chat", ProviderKind::DeepSeek),
            ("deepseek-reasoner", ProviderKind::DeepSeek),
            ("deepseek/deepseek-chat", ProviderKind::DeepSeek),
            ("wind-deepseek", ProviderKind::DeepSeek),
            ("SDU-AI/DeepSeek-V4-Flash", ProviderKind::DeepSeek),
            ("Ali-dashscope/DeepSeek-V3.2", ProviderKind::DeepSeek),
        ...
        for (model, expected_provider) in cases {
            assert_eq!(detect_provider_kind(model), expected_provider, "{model}");
            assert_eq!(
                super::metadata_for_model(model).map(|metadata| metadata.provider),
    ...
    fn maps_provider_kind_to_model_family_identity() {
        ...
        let deepseek_identity = model_family_identity_for_kind(deepseek);
        ...
        assert_eq!(deepseek_identity, runtime::ModelFamilyIdentity::Generic);
    }
    ...
    fn maps_model_name_to_model_family_identity() {
        ...
        let deepseek_model = "wind-deepseek";
        ...
        let deepseek_identity = model_family_identity_for(deepseek_model);
    ...
    fn provider_capability_matrix_snapshots_openai_compat_differences() {
        ...
        let deepseek_chat = provider_capabilities_for_model("wind-deepseek");
        assert_eq!(deepseek_chat.provider, ProviderKind::DeepSeek);
        assert_eq!(deepseek_chat.auth_env, "DEEPSEEK_API_KEY");
        assert_eq!(deepseek_chat.base_url_env, "DEEPSEEK_BASE_URL");
        assert_eq!(
            deepseek_chat.default_base_url,
            openai_compat::DEFAULT_DEEPSEEK_BASE_URL
        );
        assert_eq!(
            deepseek_chat.wire_protocol,
            ProviderWireProtocol::OpenAiChatCompletions
        );
        assert_eq!(
            deepseek_chat.reasoning_content_history,
            ProviderFeatureSupport::Unsupported
        );
    ...
    #[test]
    fn provider_diagnostics_explain_deepseek_provider() {
        let diagnostics = super::provider_diagnostics_for_model("wind-deepseek");

        assert_eq!(diagnostics.provider, ProviderKind::DeepSeek);
        assert_eq!(diagnostics.resolved_model, "deepseek/deepseek-chat");
        assert_eq!(diagnostics.auth_env, "DEEPSEEK_API_KEY");
        assert_eq!(diagnostics.base_url_env, "DEEPSEEK_BASE_URL");
        assert_eq!(
            diagnostics.default_base_url,
            openai_compat::DEFAULT_DEEPSEEK_BASE_URL
        );
        assert!(diagnostics.openai_compatible);
        assert!(!diagnostics.preserves_reasoning_content_in_history);
    ...
    fn keeps_existing_max_token_heuristic() {
        ...
        assert_eq!(max_tokens_for_model("wind-deepseek"), 32_768);
    }
    ...
    fn returns_context_window_metadata_for_supported_models() {
        ...
        assert_eq!(
            model_token_limit("wind-deepseek")
                .expect("wind-deepseek should resolve to deepseek-chat limits")
                .context_window_tokens,
            128_000
        );
    }
