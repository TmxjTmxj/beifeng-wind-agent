...
mod tests {
    ...
    #[test]
    fn deepseek_models_use_deepseek_config_not_openai() {
        let _lock = env_lock();
        let _deepseek = EnvVarGuard::set("DEEPSEEK_API_KEY", Some("test-deepseek-key"));
        let _deepseek_base = EnvVarGuard::set("DEEPSEEK_BASE_URL", None);
        let _openai = EnvVarGuard::set("OPENAI_API_KEY", None);
        ...
        for model in [
            ...
            assert!(
                client.is_ok(),
                "{model} with DEEPSEEK_API_KEY set should build successfully, got: {:?}",
                client.err()
            );

            match client.unwrap() {
                ProviderClient::DeepSeek(deepseek_client) => {
                    assert_eq!(
                        deepseek_client.base_url(),
                        "https://api.deepseek.com/v1",
                        "{model} should route to the default DeepSeek base URL"
                    );
                }
                other => panic!("Expected ProviderClient::DeepSeek for {model}, got: {other:?}"),
            }
        }
    }

    #[test]
    fn deepseek_requires_deepseek_api_key_even_when_openai_key_is_set() {
        let _lock = env_lock();
        ...
        let error = ProviderClient::from_model("deepseek-chat")
            .expect_err("DeepSeek must not read OPENAI_API_KEY");
        ...
        match error {
            ...
            other => panic!("expected DeepSeek MissingCredentials error, got {other:?}"),
        }
    }
}
