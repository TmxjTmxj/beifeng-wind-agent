use crate::{
    generate_wind_inspection_advice, generate_wind_risk_assessment, QueryResponse, RagHit,
    ScoreBreakdown, SearchMode,
};
use serde_json::json;
...
pub async fn query_qdrant(q: &[f32], top_k: u32) -> Result<Option<QueryResponse>, String> {
    ...
    let advice = generate_wind_inspection_advice("", &hits, &[]);
    let risk_assessment = generate_wind_risk_assessment(&advice, &hits);
    Ok(Some(QueryResponse {
        advice,
        risk_assessment,
        hits,
        graph_suggestions: Vec::new(),
        phase: "2-qdrant".to_string(),
