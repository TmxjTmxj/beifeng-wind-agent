...
// Region 1: CLI args / model resolution (lines ~693-2050)
//   - CliAction enum, parse_args, resolve_model_alias, etc.
//   - Candidate for extraction → cli_args.rs
// Region 2: Doctor / health checks (lines ~2350-3000)
//   - run_doctor, check_*_health functions
//   - Partially extracted to doctor.rs; remaining could join it
// Region 3: Export / session rendering (lines ~8760-8926)
//   - render_export_text, run_export, etc.
//   - Candidate for extraction → export.rs
// Region 4: Tool formatting (lines ~10380-10769)
//   - format_tool_call_start, format_bash_call, format_*_result
//   - Candidate for merging into render.rs
// Region 5: Plugin management (lines ~9036-9171)
//   - build_plugin_manager, plugins_command_payload_*
//   - Candidate for extraction → plugin_cmd.rs
// Region 6: Progress reporting (lines ~9171-9440)
//   - InternalPromptProgress* structs/impls
//   - Candidate for extraction → progress.rs
// Region 7: Runtime / client bridge (lines ~9500-10110)
//   - build_runtime, AnthropicRuntimeClient, etc.
//   - HIGH RISK — do not extract yet
// Region 8: Tool execution bridge (lines ~10927-11060)
//   - CliToolExecutor, ToolExecutor impl
//   - HIGH RISK — do not extract yet
// Region 9: Help text (lines ~11125-11297)
//   - print_help_to, print_help
//   - Candidate for extraction → help_text.rs
// ───────────────────────────────────────────────────────────────────────
...
use commands::{
    ...
    slash_command_specs, PluginsCommandResult, SkillSlashDispatch, SlashCommand,
};
use compat_harness::{extract_manifest, UpstreamPaths};
use config_cmd::{load_config_with_warning_mode, ConfigWarningMode};
use doctor::{DiagnosticCheck, DiagnosticLevel, DoctorReport};
...
use runtime::{
    ...
    McpServerManager, McpServerSpec, McpTool, MessageRole, PermissionMode, PermissionPolicy,
    ProjectContext, PromptCacheEvent, ResolvedPermissionMode, RuntimeError, Session, TokenUsage,
    ToolError, ToolExecutor, UsageTracker,
};
use serde::Deserialize;
use serde_json::{json, Map, Value};
use status::GitWorkspaceSummary;
...
impl ModelProvenance {
    ...
    #[allow(dead_code)] // TODO(phase-6): keep for future explicit model provenance reporting.
...
#[allow(dead_code)] // TODO(phase-6): reserved for local OAuth callback flow.
const DEFAULT_OAUTH_CALLBACK_PORT: u16 = 4545;
const VERSION: &str = env!("CARGO_PKG_VERSION");
const BUILD_TARGET: Option<&str> = option_env!("TARGET");
const GIT_SHA: Option<&str> = option_env!("GIT_SHA");
#[allow(dead_code)] // TODO(phase-6): used by dormant internal prompt progress runner.
const INTERNAL_PROGRESS_HEARTBEAT_INTERVAL: Duration = Duration::from_secs(3);
const POST_TOOL_STALL_TIMEOUT: Duration = Duration::from_secs(10);
const PRIMARY_SESSION_EXTENSION: &str = "jsonl";
#[allow(dead_code)] // TODO(phase-6): retained for legacy session migration compatibility.
...
#[allow(dead_code)] // TODO(phase-6): reserved for future session reference parsing.
...
fn classify_error_kind(message: &str) -> &'static str {
            ...
            || message.contains("rate limit"))
    {
        ...
        || message.contains(".claw\\settings.json")
...
#[allow(dead_code)] // TODO(phase-6): keep for JSON plugin command output parity.
...
fn resolve_model_alias(model: &str) -> &str {
    match model {
        ...
        "wind-deepseek" => "deepseek/deepseek-chat",
        "sdu-deepseek" => "SDU-AI/DeepSeek-V4-Flash",
...
fn validate_model_syntax(model: &str) -> Result<(), String> {
    ...
    if parts.len() != 2 || parts[0].is_empty() || parts[1].is_empty() {
        ...
        if trimmed.starts_with("gpt-") || trimmed.starts_with("gpt_") {
            ...
            err_msg.push_str("\nDid you mean `deepseek/");
            err_msg.push_str(trimmed);
            err_msg.push_str("`? (Requires DEEPSEEK_API_KEY env var)");
        }
        return Err(err_msg);
    }
    Ok(())
...
fn render_doctor_report(
    ...
    Ok(DoctorReport {
        checks: vec![
            check_auth_health(),
            check_deepseek_provider_health(),
...
fn check_auth_health() -> DiagnosticCheck {
    ...
    let openai_key_present = env::var("OPENAI_API_KEY")
        .ok()
        .is_some_and(|value| !value.trim().is_empty());
    let deepseek_key_present = env::var("DEEPSEEK_API_KEY")
        .ok()
        .is_some_and(|value| !value.trim().is_empty());
    let any_auth_present =
        api_key_present || auth_token_present || openai_key_present || deepseek_key_present;
    let env_details = format!(
        "Environment       api_key={} auth_token={} openai_key={} deepseek_key={}",
    ...
    match load_oauth_credentials() {
        ...
        .with_data(Map::from_iter([
            ...
            (
                "deepseek_api_key_present".to_string(),
                json!(deepseek_key_present),
            ),
...
fn check_deepseek_provider_health() -> DiagnosticCheck {
    let api_key_present = env::var("DEEPSEEK_API_KEY")
        .ok()
        .is_some_and(|value| !value.trim().is_empty());
    let base_url = api::read_deepseek_base_url();
    let status = if api_key_present {
        "OK"
    } else {
        "Missing API key"
    };
    ...
    .with_details(vec![
        format!(
            "API Key           {}",
            if api_key_present { "found" } else { "missing" }
        ),
        format!("Base URL          {base_url}"),
        "Default Model     deepseek-chat".to_string(),
        format!("Status           {status}"),
    ])
    .with_hint(if api_key_present {
        ""
    } else {
        "Set DEEPSEEK_API_KEY to use DeepSeek models such as deepseek-chat."
    })
    .with_data(Map::from_iter([
        ("api_key_present".to_string(), json!(api_key_present)),
        ("base_url".to_string(), json!(base_url)),
        ("default_model".to_string(), json!("deepseek-chat")),
        ("provider_status".to_string(), json!(status)),
    ]))
}
...
fn run_resume_command(
    ...
    let _session_list_outcome = || -> Result<ResumeCommandOutcome, Box<dyn std::error::Error>> {
 ...
 ╚═════╝╚══════╝╚═╝  ╚═╝ ╚══╝╚══╝\x1b[0m \x1b[38;5;208mCode\x1b[0m 🦞\n\n\
  ...
  Type \x1b[1m/help\x1b[0m for commands · \x1b[1m/status\x1b[0m for live context · \x1b[2m/resume latest\x1b[0m jumps back to the newest session · \x1b[1m/diff\x1b[0m then \x1b[1m/commit\x1b[0m to ship · \x1b[2mTab\x1b[0m for workflow completions · \x1b[2mShift+Enter\x1b[0m for newline",
    ...
    fn print_status(&self) {
        let status_context = match status_context(Some(&self.session.path)) {
            Ok(context) => context,
            Err(error) => {
                eprintln!("status unavailable: {error}");
                return;
            }
        };
    ...
    fn print_sandbox_status() {
        let cwd = match env::current_dir() {
            Ok(cwd) => cwd,
            Err(error) => {
                eprintln!("sandbox status unavailable: {error}");
                return;
            }
        };
        let loader = ConfigLoader::default_for(&cwd);
    ...
    #[allow(dead_code)] // TODO(phase-6): dormant internal prompt runner used by planned automation flows.
    ...
    #[allow(dead_code)] // TODO(phase-6): convenience wrapper for dormant internal prompt runner.
...
#[allow(dead_code)] // TODO(phase-6): reserved for session-management command expansion.
...
#[allow(dead_code)] // TODO(phase-6): reserved for latest-session command expansion.
...
fn render_help_topic(topic: LocalHelpTopic) -> String {
  ...
  Purpose          diagnose local auth, provider, config, workspace, sandbox, and build metadata
...
fn render_doctor_help_json() -> serde_json::Value {
    json!({
        ...
        "schema_version": "1.0",
        "usage": "claw doctor [--output-format <format>]",
        "purpose": "diagnose local auth, provider, config, workspace, sandbox, boot preflight, and build metadata",
        "formats": ["text", "json"],
        "local_only": true,
        ...
        "check_names": ["auth", "deepseek provider", "config", "install source", "workspace", "boot preflight", "sandbox", "system"],
...
fn render_config_json(
    ...
    if let Some(section) = section {
        ...
        let Some(map) = obj.as_object_mut() else {
            return Err("config report base envelope was not a JSON object".into());
        };
        map.insert(
            "section".to_string(),
...
#[allow(dead_code)] // TODO(phase-6): retained for legacy init command path.
...
#[allow(dead_code)] // TODO(phase-6): retained for future Git command preflight checks.
...
#[allow(dead_code)] // TODO(phase-6): retained for optional external tool detection.
...
#[allow(dead_code)] // TODO(phase-6): retained for draft/edit helper workflows.
...
#[allow(dead_code)] // TODO(phase-6): retained for generated issue/PR drafting context.
...
#[allow(dead_code)] // TODO(phase-6): retained for generated issue/PR drafting.
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[allow(dead_code)] // TODO(phase-6): dormant internal prompt progress events.
...
#[derive(Debug)]
#[allow(dead_code)] // TODO(phase-6): dormant internal prompt progress runner.
...
#[allow(dead_code)] // TODO(phase-6): dormant internal prompt progress reporter helpers.
...
#[allow(dead_code)] // TODO(phase-6): dormant internal prompt progress runner helpers.
...
impl AnthropicRuntimeClient {
    fn new(
        ...
        // DeepSeek variants, where xAI/OpenAI/DeepSeek use the OpenAI-compat
        // wire format under the hood. We consult
        // `detect_provider_kind(&resolved_model)` so model-name prefix
        // routing (`openai/`, `gpt-`, `grok`, `qwen/`, `deepseek-*`) wins over
        // env-var presence.
        //
...
mod tests {
    ...
    fn write_plugin_fixture(root: &Path, name: &str, include_hooks: bool, include_lifecycle: bool) {
        ...
        if include_lifecycle {
            ...
            if cfg!(windows) {
                fs::write(
                    root.join("lifecycle").join("init.cmd"),
                    "@echo off\r\necho init>> lifecycle.log\r\n",
                )
                .expect("write init lifecycle");
                fs::write(
                    root.join("lifecycle").join("shutdown.cmd"),
        ...
        let lifecycle = if include_lifecycle && cfg!(windows) {
            ",\n  \"lifecycle\": {\n    \"Init\": [\"./lifecycle/init.cmd\"],\n    \"Shutdown\": [\"./lifecycle/shutdown.cmd\"]\n  }"
        } else if include_lifecycle {
    ...
    #[test]
    fn parses_wind_report_generate_allowed_tool() {
        let _guard = env_lock();
        std::env::remove_var("RUSTY_CLAUDE_PERMISSION_MODE");
        let args = vec![
            "--allowedTools".to_string(),
            "wind_report_generate".to_string(),
            "生成叶片裂纹报告".to_string(),
        ];
        assert_eq!(
            parse_args(&args).expect("wind_report_generate should be allow-listable"),
            CliAction::Prompt {
                prompt: "生成叶片裂纹报告".to_string(),
                model: DEFAULT_MODEL.to_string(),
                output_format: CliOutputFormat::Text,
                allowed_tools: Some(
...
"#,
        ...
        let expected_cwd = cwd.canonicalize().unwrap_or(cwd.clone());
        assert_eq!(
            context.cwd.to_string_lossy().trim_start_matches(r"\\?\"),
            expected_cwd.to_string_lossy().trim_start_matches(r"\\?\")
        );
        assert_eq!(
    ...
    fn classify_error_kind_returns_correct_discriminants() {
        ...
        assert_eq!(
            classify_error_kind(
                r#"C:\tmp\project\.claw\settings.json: field "model" must be a string, got a number"#,
            ),
            "config_parse_error"
        );
        assert_eq!(
    ...
    #[test]
    fn filtered_tool_specs_include_wind_report_generate_when_allowed() {
        let allowed = ["wind_report_generate"]
            .into_iter()
            .map(str::to_string)
            .collect();
        let filtered = filter_tool_specs(&GlobalToolRegistry::builtin(), Some(&allowed));
        let names = filtered
            ...
            .map(|spec| spec.name)
            .collect::<Vec<_>>();
        assert_eq!(names, vec!["wind_report_generate"]);
    }
...
UU conflicted.rs",
    ...
    fn build_runtime_plugin_state_discovers_mcp_tools_and_surfaces_pending_servers() {
        ...
        let (python_command, alpha_args, broken_args) = if cfg!(windows) {
            (
                "py",
...
mod alias_resolution_tests {
    ...
    fn test_alias_resolution_builtin() {
        ...
        assert_eq!(
            resolve_model_alias_with_config("wind-deepseek"),
            "deepseek/deepseek-chat"
        );
        assert_eq!(
            resolve_model_alias_with_config("sdu-deepseek"),
            "SDU-AI/DeepSeek-V4-Flash"
        );
        assert_eq!(
            resolve_model_alias_with_config("deepseek-chat"),
            "deepseek/deepseek-chat"
        );
        assert_eq!(
            resolve_model_alias_with_config("deepseek-reasoner"),
    ...
    fn test_alias_resolution_syntax_validation() {
        ...
        let deepseek = resolve_model_alias_with_config("wind-deepseek");
        assert!(validate_model_syntax(&deepseek).is_ok());
    }
