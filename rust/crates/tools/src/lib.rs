...
#[cfg(test)]
use wind::{
    wind_knowledge_query_request, wind_report_markdown, WindKnowledgeQueryInput,
...
fn execute_tool_with_enforcer(
    ...
    match name {
        ...
        "wind_knowledge_query" | "wind_fault_analysis" | "wind_report_generate" => {
            wind::execute(name, input)
        }
...
mod tests {
    ...
    #[test]
    fn wind_knowledge_query_is_registered_with_expected_schema() {
        let spec = mvp_tool_specs()
            .into_iter()
            .find(|spec| spec.name == "wind_knowledge_query")
            .expect("wind_knowledge_query should be registered");

        assert!(spec.description.contains("wind turbine inspection"));
        assert_eq!(spec.required_permission, PermissionMode::ReadOnly);
        assert!(spec.input_schema["properties"].get("query").is_some());
        assert!(spec.input_schema["properties"].get("component").is_some());
        assert!(spec.input_schema["properties"].get("symptom").is_some());
        assert!(spec.input_schema["properties"].get("domain").is_some());
        assert!(spec.input_schema["properties"].get("equipment").is_some());
        assert!(spec.input_schema["properties"].get("top_k").is_some());
        assert!(spec.input_schema["properties"].get("debug").is_some());
    }

    #[test]
    fn wind_fault_analysis_is_registered_with_expected_schema() {
        let spec = mvp_tool_specs()
            .into_iter()
            .find(|spec| spec.name == "wind_fault_analysis")
            .expect("wind_fault_analysis should be registered");

        assert!(spec.description.contains("Wind Fault Analysis"));
        assert_eq!(spec.required_permission, PermissionMode::ReadOnly);
        assert!(spec.input_schema["properties"].get("problem").is_some());
        assert!(spec.input_schema["properties"].get("component").is_some());
        assert!(spec.input_schema["properties"].get("symptom").is_some());
    }

    #[test]
    fn wind_report_generate_is_registered_with_expected_schema() {
        let spec = mvp_tool_specs()
            .into_iter()
            .find(|spec| spec.name == "wind_report_generate")
            .expect("wind_report_generate should be registered");

        assert!(spec.description.contains("Markdown Wind O&M report"));
        assert_eq!(spec.required_permission, PermissionMode::WorkspaceWrite);
        assert!(spec.input_schema["properties"].get("problem").is_some());
        assert!(spec.input_schema["properties"].get("component").is_some());
        assert!(spec.input_schema["properties"].get("symptom").is_some());
        assert!(spec.input_schema["properties"].get("report_type").is_some());
        assert!(spec.input_schema["properties"].get("title").is_some());
    }

    #[test]
    fn wind_knowledge_query_normalizes_chinese_blade_crack() {
        let request = super::wind_knowledge_query_request(&super::WindKnowledgeQueryInput {
            query: "无人机巡检发现叶片疑似裂纹".to_string(),
            component: Some("叶片".to_string()),
            symptom: None,
            domain: Some("巡检与故障诊断".to_string()),
            equipment: Some("风力发电机组".to_string()),
            top_k: Some(5),
            debug: None,
        });

        assert_eq!(request["component"], "Blade");
        assert_eq!(request["domain"], "Blade");
        assert_eq!(request["equipment"], "blade");
        assert_eq!(request["symptom"], "裂纹");
        assert!(request["query"].as_str().unwrap().contains("裂纹"));
    }

    #[test]
    fn wind_knowledge_query_final_request_body_overwrites_chinese_blade_crack_inputs() {
        let request = super::wind_knowledge_query_request(&super::WindKnowledgeQueryInput {
            query: "无人机巡检发现叶片疑似裂纹".to_string(),
            component: Some("叶片".to_string()),
            symptom: Some("疑似裂纹".to_string()),
            domain: None,
            equipment: Some("风力发电机组".to_string()),
            top_k: Some(5),
            debug: None,
        ...
        assert_eq!(request["equipment"], "blade");
        assert_eq!(request["symptom"], "裂纹");
        assert_ne!(request["component"], "叶片");
        assert_ne!(request["equipment"], "风力发电机组");
        assert_ne!(request["symptom"], "疑似裂纹");
    }

    #[test]
    fn wind_knowledge_query_normalizes_chinese_gearbox_oil_temperature() {
        let request = super::wind_knowledge_query_request(&super::WindKnowledgeQueryInput {
            query: "齿轮箱油温持续升高怎么排查".to_string(),
            component: Some("齿轮箱".to_string()),
            symptom: None,
            domain: None,
            equipment: Some("齿轮箱".to_string()),
            top_k: None,
            debug: None,
        });

        assert_eq!(request["component"], "Gearbox");
        assert_eq!(request["domain"], "Gearbox");
        assert_eq!(request["equipment"], "gearbox");
        assert_eq!(request["symptom"], "油温升高");
        assert!(request["query"].as_str().unwrap().contains("油温升高"));
    }

    #[test]
    fn wind_knowledge_query_normalizes_chinese_power_curve_abnormal() {
        let request = super::wind_knowledge_query_request(&super::WindKnowledgeQueryInput {
            query: "功率曲线异常并伴随报警".to_string(),
            component: None,
            symptom: None,
            domain: Some("运行状态分析".to_string()),
            equipment: None,
            top_k: None,
            debug: None,
        });

        assert_eq!(request["component"], "SCADA");
        assert_eq!(request["domain"], "SCADA");
        assert_eq!(request["equipment"], "scada");
        assert_eq!(request["symptom"], "功率曲线异常");
        assert!(request["query"].as_str().unwrap().contains("功率曲线异常"));
    }

    #[test]
    fn wind_knowledge_query_calls_rag_service_and_returns_core_fields() {
        let _guard = env_lock()
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
        let server = TestServer::spawn(Arc::new(|request_line: &str| {
            assert!(request_line.contains("POST /v1/query "));
            HttpResponse::text(
                200,
                "OK",
                r#"{
        ...
        std::env::set_var("CLAW_RAG_SERVICE_URL", format!("http://{}", server.addr()));
        let result = execute_tool(
        ...
        .expect("wind knowledge query should succeed");
        std::env::remove_var("CLAW_RAG_SERVICE_URL");
        ...
        assert_eq!(
            output["risk_assessment"]["shutdown_evaluation_required"],
            true
        );
    }

    #[test]
    fn wind_knowledge_query_debug_shows_normalized_final_request_body() {
        let _guard = env_lock()
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
        let server = TestServer::spawn(Arc::new(|request_line: &str| {
            ...
            HttpResponse::text(
                ...
                r#"{
                    ...
                    "risk_assessment": {"shutdown_evaluation_required": true}
                }"#,
            )
        }));
        ...
        let result = execute_tool(
            "wind_knowledge_query",
            &json!({
                "query": "无人机巡检发现叶片疑似裂纹，是否需要停机？多久复检？",
                "component": "叶片",
                "equipment": "风力发电机组",
                "symptom": "疑似裂纹",
                "debug": true
        ...
        let output: serde_json::Value = serde_json::from_str(&result).expect("valid json");
        let body = &output["debug"]["post"]["body"];
        assert_eq!(body["component"], "Blade");
        assert_eq!(body["domain"], "Blade");
        assert_eq!(body["equipment"], "blade");
        assert_eq!(body["symptom"], "裂纹");
    }

    #[test]
    fn wind_knowledge_query_reports_service_unreachable() {
        let _guard = env_lock()
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
        std::env::set_var("CLAW_RAG_SERVICE_URL", "http://127.0.0.1:9");
        let error = execute_tool(
            "wind_knowledge_query",
            &json!({
                "query": "齿轮箱油温升高怎么排查"
        ...
        .expect_err("unreachable service should fail clearly");
        std::env::remove_var("CLAW_RAG_SERVICE_URL");

        assert!(
            error.contains("claw-rag-service is not reachable"),
            "unexpected error: {error}"
        );
        assert!(error.contains("cargo run --manifest-path"));
    }

    #[test]
    fn wind_fault_analysis_wraps_knowledge_query_response() {
        let _guard = env_lock()
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
        let server = TestServer::spawn(Arc::new(|request_line: &str| {
            ...
            HttpResponse::text(
                ...
                r#"{
                    ...
                    "advice": {
                        "problem_summary": "建议检测",
                        "inspection_items": ["裂纹长度"],
                        "inspection_methods": ["无人机复检"],
                        "recommended_interval": "30 天",
                        "maintenance_actions": ["建立跟踪工单"],
                        "safety_notes": ["登塔需工作票"],
                        "missing_data": ["缺陷位置"],
                        "confidence": 0.62
                    },
        ...
        .expect("wind fault analysis should succeed");
        std::env::remove_var("CLAW_RAG_SERVICE_URL");

        let output: serde_json::Value = serde_json::from_str(&result).expect("valid json");
        assert_eq!(output["risk_level"], "Medium");
        assert_eq!(output["shutdown_evaluation_required"], true);
        assert_eq!(output["human_confirmation_required"], true);
        assert_eq!(output["inspection_items"][0], "裂纹长度");
        assert_eq!(
            output["evidence_summary"]["graph_nodes"][0],
            "Blade:叶片裂纹"
        );
    }

    #[test]
    fn wind_report_generate_creates_blade_inspection_report_file() {
        let _guard = env_lock()
            .lock()
            .unwrap_or_else(std::sync::PoisonError::into_inner);
        let server = TestServer::spawn(Arc::new(|request_line: &str| {
            ...
            HttpResponse::text(
                ...
                r#"{
                    ...
                    "graph_suggestions": [{"component": "Blade", "symptom": "叶片裂纹"}],
                    "advice": {
                        "inspection_items": ["裂纹长度", "裂纹宽度"],
                        "inspection_methods": ["无人机复检", "超声检测"],
                        "recommended_interval": "30 天",
                        "maintenance_actions": ["建立跟踪工单"],
                        "safety_notes": ["高空作业需工作票"],
                        "missing_data": ["裂纹位置"],
                        "confidence": 0.72
                    },
                    "risk_assessment": {
        ...
        let root = temp_path("wind_report_generate_blade");
        ...
        .expect("wind report generation should succeed");
        ...
        assert!(report_path.starts_with("beifeng/reports/generated/wind_report_"));
        assert!(root.join(report_path).is_file());
        let markdown = output["report_markdown"].as_str().expect("markdown");
        assert!(markdown.contains("叶片裂纹巡检分析报告"));
        assert!(markdown.contains("风险等级：Medium"));
        assert!(markdown.contains("是否需要停机评估：是"));
        assert_eq!(output["fault_analysis"]["risk_level"], "Medium");
    }

    #[test]
    fn wind_report_generate_gearbox_fault_report_contains_high_risk_fields() {
        let markdown = super::wind_report_markdown(
            &super::WindReportGenerateInput {
            ...
            &json!({
                "problem_summary": "齿轮箱油温升高需要当天复核。",
                "possible_causes": ["润滑、冷却或传动链状态异常。"],
                "inspection_items": ["油温趋势", "油样", "振动", "润滑系统"],
                "inspection_methods": ["SCADA 趋势对比", "油样检测"],
                "recommended_interval": "当天复核",
                "maintenance_actions": ["检查冷却系统", "检查润滑状态"],
                "risk_level": "High",
                "shutdown_evaluation_required": true,
                "human_confirmation_required": true,
                "safety_notes": ["不得绕过保护"],
                "evidence_summary": {
                    "hit_documents": ["knowledge_base/manuals/gearbox_temp_manual.md"],
                    "graph_nodes": ["Gearbox:齿轮箱油温升高"],
                    "risk_level": "High"
                },
                "missing_data": ["油样结果"],
                "confidence": 0.7
        ...
        assert!(markdown.contains("油温趋势"));
        assert!(markdown.contains("检查冷却系统"));
    }

    #[test]
    fn wind_report_generate_scada_risk_report_contains_human_confirmation_and_reasons() {
        let markdown = super::wind_report_markdown(
            &super::WindReportGenerateInput {
            ...
            &json!({
                "problem_summary": "功率曲线异常需要复核 SCADA 趋势。",
                "possible_causes": ["测量数据质量、限功率策略、偏航误差或设备输出受限。"],
                "inspection_items": ["风速功率趋势", "报警记录"],
                "inspection_methods": ["SCADA 趋势分析"],
                "recommended_interval": "当天复核",
                "maintenance_actions": ["核对测风和限功率状态"],
                "risk_level": "Medium",
                "shutdown_evaluation_required": false,
                "human_confirmation_required": true,
                "safety_notes": ["涉及并网策略需人工确认"],
                "evidence_summary": {
                    "hit_documents": ["knowledge_base/scada_rules/power_curve_alarm.md"],
                    "graph_nodes": ["SCADA:功率曲线异常"],
                    "risk_level": "Medium"
                },
                "missing_data": ["风速功率时间序列"],
                "confidence": 0.66
            }),
            "20260603_213000",
        );
        ...
        assert!(markdown.contains("测量数据质量、限功率策略"));
    }
